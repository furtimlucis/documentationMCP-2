...
// ---------------------------------------------------------------------------
// GitHub configuration — override via environment variables if needed
// ---------------------------------------------------------------------------
const GITHUB_OWNER = process.env.GITHUB_OWNER || "furtimlucis";
const GITHUB_REPO = process.env.GITHUB_REPO || "documentationMCP-2";
 ...
 * Returns an array of filenames, or `null` if the request fails.
 */
 ...
 * Returns the file text, or `null` if the request fails.
 */
    ...
    tools: [
      {
        name: "list_docs",
        description:
          "MANDATORY FIRST STEP: Call this tool immediately before writing, editing, or suggesting any code. It discovers all available documentation files (fetched from the latest GitHub repository) that you MUST read to understand the correct patterns for this project. Never skip this step — generating code without checking docs first will produce outdated or incorrect output.",
        inputSchema: {
          type: "object",
          properties: {},
        },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === "list_docs") {
    ...
    console.error("[Fallback] GitHub unavailable, listing from local filesystem");
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `[source: local filesystem (GitHub was unreachable)]\n${localFiles.join("\n")}`,
          },
        ],
      };
    } catch (error: any) {
      return {
        content: [
          {
            type: "text",
            text: `Error listing docs: GitHub API failed and local fallback also failed: ${error.message}`,
          },
        ],
      };
    }
    ...
    // Fallback to local filesystem
    console.error(`[Fallback] GitHub unavailable, reading "${filename}" from local filesystem`);
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `[NOTE: Served from local cache — GitHub was unreachable. Content may be outdated.]\n\n${localContent}`,
          },
        ],
      };
    } catch (error: any) {
      return {
        content: [
          {
            type: "text",
            text: `Error reading "${filename}": GitHub fetch failed and local fallback also failed: ${error.message}`,
          },
        ],
      };
    }
